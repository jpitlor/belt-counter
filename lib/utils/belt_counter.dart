...
markPicture(String imagePath) {
  ...
  for (int x = marker.item1 - ppi; x < marker.item1; x++) {
    ...
    drawRect(
        image,
        x,
        (marker.item2 + (ppi / 2)).floor(),
        x,
        marker.item2 + ppi,
        isWhite ? Color.fromRgb(255, 255, 255) : Color.fromRgb(0, 0, 0));
    drawRect(image, x, marker.item2 + 10, x, (marker.item2 + (ppi / 2)).floor(),
        image.getPixel(x, 2));
  }

  drawRect(image, marker.item1, marker.item2, marker.item3, marker.item4,
      Color.fromRgb(255, 0, 0));
  drawRect(image, marker.item1 - ppi, marker.item2, marker.item1,
      marker.item2 + ppi, Color.fromRgb(0, 255, 0));
...
Image _getOneSquareInchOfBelt(Image image) {
  ...
  return copyRotate(
      copyCrop(image, marker.item1 - ppi, marker.item2, ppi, ppi), 90);
}
